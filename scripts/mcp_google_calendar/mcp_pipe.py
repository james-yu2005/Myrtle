...
Usage:
    ./start.sh
    python mcp_pipe.py
    python mcp_pipe.py google-calendar
"""
...
def _has_oauth_client_config() -> bool:
    if os.environ.get("GOOGLE_CLIENT_ID", "").strip() and os.environ.get(
    ...
    if explicit:
        ...
        return path.is_file()
    return (Path(SCRIPT_DIR) / "credentials.json").is_file()
...
if __name__ == "__main__":
    ...
    if not endpoint_url:
        logger.error(
            "Set MCP_ENDPOINT in scripts/mcp_google_calendar/.env "
            "(copy from .env.example)"
        )
        sys.exit(1)

    if not _has_oauth_client_config():
        logger.error(
            "Configure Google OAuth: set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET "
            "in .env, or add credentials.json in scripts/mcp_google_calendar/"
        )
        sys.exit(1)
